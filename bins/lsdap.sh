...
help() {
    echo "Usage: lsdap [option] [arguments]"
    echo "Options:"
    echo "  -ls [object]          List objects in LDAP domain. Object can be 'ou', 'user' or 'group'."
    echo "  -new [object] [name]  Create a new object in LDAP domain. Object can be 'ou', 'user' or 'group'. Name is required."
    echo "  -mv [object] [name]   Move an object to another location in LDAP domain. Object can be 'ou', 'user' or 'group'. Name is required."
    echo "  -rm [object] [name]   Delete an object from LDAP domain. Object can be 'ou', 'user' or 'group'. Name is required."
    echo "  -uninstall            Uninstall the LDAP domain and all its data."
    echo "  -ssh [username]       Configure SSH access for a user in LDAP domain."
    echo "  -ad [username]        Configure AnyDesk access for a user in LDAP domain."
    echo "  -help                 Display this help message."

}

if [[ $1 = "-ls" ]]; then
    $lsdapbins/lsdapget.sh $2 $3
elif [[ $1 = "-new" ]]; then
    $lsdapbins/lsdapnew.sh $2 $3
elif [[ $1 = "-mv" ]]; then
    $lsdapbins/lsdapmove.sh $2 $3
elif [[ $1 = "-rm" ]]; then
    $lsdapbins/lsdapdel.sh $2 $3
elif [[ $1 = "-uninstall" ]]; then
    $lsdapbins/lsdapuninstall.sh
elif [[ $1 = "-ssh" ]]; then
    $lsdapbins/ssh.sh $2
elif [[ $1 = "-ad" ]]; then
    $lsdapbins/AnyDesk.sh $2
elif [[ $1 = "-help" ]]; then
    help
else
